...
import { formatNumberPrice } from '../../utils/formatNumber';
...
export default function ModalInvoice({
  ...
  const handleSubmit = async (e) => {
    ...
    const validator = modalInvoiceValidator(submit);
    ...
    if (validator.error) {
      ...
      data.variants.forEach((item) => (item.status = true));
  ...
  useEffect(() => {
    ...
    getByUserId(uuid, token).then((data) => setStaff(data));
  }, []);
  ...
  useEffect(() => {
    if (details) {
      const total = details.reduce((prev, curr) => {
        let totalQuantity;
        if (curr.has_size)
          totalQuantity = curr.list_sizes.reduce(
            (prev2, cur2) => (prev2 += Number(cur2.quantity)),
            0
          );
        else
          totalQuantity = Number(curr.quantityNoSize);
        const total =
          totalQuantity * Number(curr.invoice_price.replace(/,/g, ''));

        return (prev += total);
      }, 0);

      setTotal(formatNumberPrice(total + ''));
    }
  }, [details]);

  useEffect(() => {
    if (data) {
      const arr = data.variants.reduce((prev, curr) => {
                  ...
                  <div className='w-full lg:w-1/2 px-4'>
                    <div className='relative w-full mb-3'>
                      ...
                      <input
                        type='text'
                        className='border-0 px-3 py-3 placeholder-slate-200 text-slate-800 font-semibold bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150 h-[44px]'
                        value={data?.product_name + ' | ID:' + data?.id}
                        disabled
                      />
                      <small className='text-red-500 font-medium'>
                        {messages.map((message) =>
                        ...
                        {details &&
                          details.map((item, index) => (
                            ...
                            >
                              ...
                              <div>
                                Quantity:{' '}
                                {item.has_size ? item.list_sizes.reduce(
                                  (prev, curr) =>
                                    (prev += Number(curr.quantity)),
                                  0
                                ) : Number(item.quantityNoSize)}
                              </div>
                            </div>
                          ))}
                      </div>
