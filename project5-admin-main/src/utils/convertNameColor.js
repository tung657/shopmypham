...
var ntc = {
  ...
  name: function (color) {
    ...
    if (color.length < 3 || color.length > 7)
      return {hex: '#000000', color: 'Invalid Color: ' + color, status: false};
    ...
    for (var i = 0; i < ntc.names.length; i++) {
      if (color === '#' + ntc.names[i][0])
        return {hex: '#' + ntc.names[i][0], color: ntc.names[i][1], status: true};
    ...
    const result = {
      hex: cl < 0 ? '#000000' : '#' + ntc.names[cl][0],
      color: cl < 0 ? 'Invalid Color: ' + color : ntc.names[cl][1],
      status: false
    }
